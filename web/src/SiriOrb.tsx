// Vendored from smoothui (https://smoothui.dev/docs/components/siri-orb).
//
// Copied rather than installed via `npx shadcn add @smoothui/siri-orb`: that
// CLI insists on a components.json + Tailwind + tailwind-merge init, and this
// component uses zero Tailwind classes — its entire visual is the conic
// gradients in the <style> block below. The library's only ecosystem touch was
// `cn("siri-orb", className)`, which here is joining two strings, so it's
// inlined and the dependency dropped. Otherwise kept as upstream wrote it, to
// keep future diffs against the registry readable.
